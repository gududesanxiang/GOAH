...
import numpy as np
from sklearn.cluster import KMeans
from nan import find_NaN_auto   # the ONLY correct Natural Neighbor function
...
def build_dynamic_hypergraph(
    ...
    """
    Build hypergraph using:
      (1) Natural Neighbors (correct nan.py implementation)
      (2) KMeans cluster hyperedges

    Returns:
        H : (n, m)
        W : (n, n)  = H * D_e^-1 * H^T
        hyperedge_sizes
        NaN_adj : natural neighbor adjacency (debug)
    """
    X = data
    n = X.shape[0]

    # ============================================================
    # (1) Natural Neighbor Local Graph
    # ============================================================
    if use_natural_neighbor:
        # auto minK/maxK
        minK = 2
        # maxK = min(max(10, int(np.sqrt(n))), n - 1)
        maxK = n-1

        NaN_adj, NaN_distance = find_NaN_auto(X, minK, maxK)
        # make symmetric
        NaN_adj = (NaN_adj + NaN_adj.T) / 2
        local_adj = torch.tensor(NaN_adj, dtype=torch.float32)
    else:
        raise ValueError("Natural Neighbors must be enabled.")

    # local hyperedges: each node's natural neighbor set
    local_hyperedges = []
    for i in range(n):
        nbrs = torch.where(local_adj[i] > 0)[0].tolist()
        if len(nbrs) == 0:
            # natural neighbors failed → fallback: self-loop
            nbrs = [i]
        else:
            if i not in nbrs:
                nbrs.append(i)
        local_hyperedges.append(nbrs)

    # ============================================================
    # (2) KMeans cluster hyperedges (unchanged)
    # ============================================================
    cluster_hyperedges = []
    if n_clusters > 0:
        ...
        for c in range(n_clusters):
            ...
            if len(nodes) < 2:
                continue
            for _ in range(S):
                cluster_hyperedges.append(nodes)

    # ============================================================
    # merge hyperedges
    # ============================================================
    all_edges = local_hyperedges + cluster_hyperedges
    m = len(all_edges)

    H = torch.zeros((n, m), dtype=torch.float32)
    hyperedge_sizes = []

    for e_idx, nodes in enumerate(all_edges):
        hyperedge_sizes.append(len(nodes))
        for v in nodes:
            H[v, e_idx] = 1.0

    hyperedge_sizes = torch.tensor(hyperedge_sizes, dtype=torch.float32)

    # ============================================================
    # build W = H * D_e^-1 * H^T
    # ============================================================
    inv_sizes = 1.0 / hyperedge_sizes
    ...
    return {
        "H": H.to(device),
        "W": W.to(device),
        "hyperedge_sizes": hyperedge_sizes.to(device),
        "NaN_adj": local_adj.to(device),
    }
